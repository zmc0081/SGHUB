...
- `data/sghub.db` — SQLite 数据库(文献元数据、Chat 历史、解析结果、订阅设置、PDF 阅读标注)
...
当你配置 Anthropic / OpenAI / DeepSeek / Google Vertex / Ollama / 其他 OpenAI 兼容服务时:
...
**Google Vertex(ADC 免密,v2.2.8 起)**:选择 ADC 认证方式时,SG Hub 读取你本机的
Google 凭证(gcloud 登录生成的 Application Default Credentials 或
`GOOGLE_APPLICATION_CREDENTIALS` 指向的服务账号文件),向 Google OAuth
(`oauth2.googleapis.com`)换取短期访问令牌后调用 Vertex AI。凭证文件与令牌**只在本机内存中使用**,
不写入 SG Hub 的任何文件或日志,更不会发送给 SG Hub 或第三方;若配置了代理,令牌与模型请求经该代理直达 Google。
...
- **划选翻译 / 划选 Ask(v2.2.10 起)**:在阅读器中划选文本后使用"翻译"或"Ask 大模型"时,仅将**选中的文本片段**(Ask 另附文献标题与你的问题)发送给你配置的模型,数据流向与上述翻译/Chat 一致;高亮/划线标注只存本地数据库,不发送给任何一方
...
| 流向 | 触发 | 数据 |
|---|---|---|
| → 模型提供商(Anthropic / OpenAI / DeepSeek / Google Vertex / Ollama 等) | AI 解析 / Chat / Skill 测试 / 全文翻译 | 你选定的论文 / PDF 文本 + prompt + 模型参数 |
