...
SG Hub 是一款开源(MIT)桌面客户端,面向科研工作者,把"找文献、管文献、读文献"整合到一个本地优先、隐私可控的工作台中。支持多源聚合检索、关键词订阅推送、文献数据库管理、AI 文献解析、Chat 自由对话,以及 BYOK 多模型配置。
...
- **文献数据库** — 多级文件夹 + 标签 + 智能文件夹 + 本地 PDF 上传与集中管理
- **内置 PDF 阅读器** — 软件内打开 PDF,翻页/缩放/目录/搜索
- **全文翻译** — 借助大模型,保持结构与排版的学术级翻译
- **AI 文献解析** — 多 Skill 结构化精读,流式输出
- **Chat 自由对话** — 多模型切换 + 附件 / Skill / 文献引用
- **模型配置(BYOK)** — Claude / GPT / DeepSeek / Ollama 本地;API Key 仅存系统密钥链
- **AI Store** — 对接独立的 SG AI Store,购买配额即买即用
...
![SG Hub 检索层目标架构](./sghub_retrieval_layer_architecture.png)
...
| PDF | pdf.js(渲染)+ pdf-extract / lopdf(文本提取) |
