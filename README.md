...
>
> **AI-powered academic literature manager for researchers.** Open source, local-first, BYOK + optional prepaid models.
>
> 项目代号:`SGHUB`(GitHub 仓库名、包名、bundle id 等技术标识符保留),展示名统一为 **SG Hub**。

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)
[![Version](https://img.shields.io/badge/version-v2.2.1-1F2E4D)](src-tauri/tauri.conf.json)
...
> 设计规范见 [`docs/ui-design/`](docs/ui-design/)(V2.2 SGHUB Capsule);
> 全部画面占位清单见 [`docs/ui-screenshots-checklist.md`](docs/ui-screenshots-checklist.md);
> V2.1 历史 UI 基线见 [`docs/ui-design-requirements.md`](docs/ui-design-requirements.md)。

| 文献数据库 | AI 解析 | AI 生成 Skill | AI Store |
|---|---|---|---|
| ![library](docs/img/screenshot-library.png) | ![parse](docs/img/screenshot-parse.png) | ![skillgen](docs/img/screenshot-skillgen.png) | ![store](docs/img/screenshot-store.png) |
...
- **成本追踪 + 图表升级** — 按模型记录 input/output token,自动计算 USD;Models 页 4 张统计卡 + **AreaChart 平滑曲线 + 7d/30d/自定义范围切换**(replaced BarChart in V2.2.1)+ X 轴 tick 自适应
...
- **Windows 10/11** — `SG Hub_2.2.1_x64-setup.exe`(NSIS 安装器)
- **macOS 12+** — `SG Hub_2.2.1_universal.dmg`(Intel & Apple Silicon 通用)
...
| **SG AI Store**(V2.2.1)| `https://sgaistore.com/v1` | <https://sgaistore.com/> | 预付费,购买即用,内置余额徽章 + 拦截 |
...
V2.2.1 引入的可选服务,与 BYOK 完全独立 — 你可以两种模式混用。
...
| Rust | **≥ 1.80**(V2.2.1 用了 `std::sync::LazyLock`)| `winget install Rustlang.Rustup` 后 `rustup default stable` |
...
│   │   └── store/                 # V2.2.1 — StoreHome / ProductDetail / mockData
│   ├── components/                # UI 组件
│   │   ├── Sidebar / Titlebar / PaperPicker / PaperActions
│   │   ├── FavoriteButton / PaperMetadataEditor / SkillEditor
│   │   ├── DataDirCard / UpdaterCard
│   │   ├── PrivacyPolicyDialog        # V2.2.1
│   │   ├── InsufficientBalanceDialog  # V2.2.1
│   │   └── chat/*
│   ├── stores/                    # Zustand: chatStore / libraryStore / skillGeneratorStore
│   ├── lib/
│   │   ├── tauri.ts               # 集中封装 invoke() 调用
│   │   ├── sgAiStoreApi.ts        # V2.2.1 — AI Store 前端契约
│   │   └── version.ts             # V2.2.1 — APP_VERSION 单源(读 package.json)
│   ├── assets/privacy/            # V2.2.1 — zh-CN.md + en-US.md (bundled)
│   ├── i18n/                      # i18next 配置
│   └── styles/                    # Tailwind + CSS 变量(主题)
├── src-tauri/
│   ├── src/
│   │   ├── ai_client/             # OpenAI 兼容 / Anthropic / Ollama 三套实现 + usage 统计
│   │   ├── ai_store/              # V2.2.1 — products / sync_strategy / sse_listener / billing / commands
│   │   ├── chat/                  # 会话 / 消息 / 附件
...
提交 PR 前请确保:
- `cargo fmt` + `cargo clippy --all-targets -- -D warnings` 通过(注意 V2.2.1 起 MSRV 升至 1.80)
- `cargo test` 全部通过(当前 248 测试:240 单元 + 8 集成)
- `npm run build` + `npx eslint src/` 通过
- 5 项 PR-blocking grep 全 0(见 [CLAUDE.md](CLAUDE.md) §UI 设计规范):无 V2.1 stale token / 无 `transition-all` / 无 `window.confirm/prompt/alert` / 无 emoji(Skill icon 例外)/ 无硬编码色值(`src/styles/index.css` 例外)
